...
def _random_data(data_type: str) -> str:
    ...
    if dt == "birthday":
        y = random.randint(1980, 2000); m = random.randint(1, 12); d = random.randint(1, 28)
...
# ── Hotkey handler with volume/brightness routing ─────────────────────────────

# Keys that pyautogui cannot handle as raw hotkeys — route to computer_settings
_SETTINGS_KEY_MAP = {
    # Volume variants
    "volume_up":           "volume_up",
    "volumeup":            "volume_up",
...
def _hotkey(keys_raw) -> str:
    """
    Handle a hotkey request.

    If the key string is a media/system key that pyautogui cannot handle
    (volume, brightness, etc.), route it to computer_settings instead.
    Otherwise pass it straight to pyautogui.hotkey().
    """
    _require_pyautogui()

    # Normalise to a list of key tokens
    if isinstance(keys_raw, str):
        keys = [k.strip().lower() for k in keys_raw.split("+")]
    else:
        keys = [str(k).strip().lower() for k in keys_raw]

    # Single-key routing check
    if len(keys) == 1:
        key_norm = keys[0].replace("-", "_")
        if key_norm in _SETTINGS_KEY_MAP:
            action = _SETTINGS_KEY_MAP[key_norm]
            return _route_to_settings(action)

    # Multi-key: check if any token is a media key we know about
    combined = "+".join(keys).replace("-", "_")
    if combined in _SETTINGS_KEY_MAP:
        return _route_to_settings(_SETTINGS_KEY_MAP[combined])

    # Normal pyautogui hotkey
    pyautogui.hotkey(*keys)
    return f"Hotkey: {'+'.join(keys)}"


def _route_to_settings(action: str) -> str:
    """Call computer_settings for actions it owns (volume, brightness, etc.)."""
    from actions.computer_settings import computer_settings
    result = computer_settings(parameters={"action": action})
    return result or f"Done: {action}"


def _press(key: str) -> str:
    _require_pyautogui()
    # Also intercept media press calls
    key_norm = key.strip().lower().replace("-", "_")
    if key_norm in _SETTINGS_KEY_MAP:
        return _route_to_settings(_SETTINGS_KEY_MAP[key_norm])
    pyautogui.press(key)
...
def computer_control(
    ...
    parameters keys (all optional unless noted):
      ...
      keys          : hotkey string, e.g. 'ctrl+c' or 'volume_down'
    ...
    Actions:
      ...
      hotkey        — key combination (volume/brightness keys auto-routed)
      press         — single key (volume/brightness keys auto-routed)
