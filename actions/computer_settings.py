...
def volume_set(value: int):
    ...
    if _OS == "Windows":
        try:
            ...
            vol_db    = -65.25 if value == 0 else max(-65.25, 20 * __import__("math").log10(value / 100))
...
def _win_brightness_get() -> int:
    """Returns current Windows brightness 0-100, or -1 on failure."""
    try:
        result = subprocess.run(
...
def _win_brightness_set(level: int):
    """Sets Windows brightness 0-100 via WMI."""
    ...
    if _OS == "Darwin":
        ...
        try:
            ...
            if current >= 0:
                ...
                # Fallback: try screen_brightness_control library
                import screen_brightness_control as sbc
                cur = sbc.get_brightness()[0]
                sbc.set_brightness(min(100, cur + 10))
        except ImportError:
            # Last resort: WMI direct call
            subprocess.run(
                ["powershell", "-NoProfile", "-Command",
            ...
            print(f"[Settings] brightness_up Windows failed: {e}")
...
def brightness_down():
    if _OS == "Darwin":
        ...
        try:
            ...
            print(f"[Settings] brightness_down Windows failed: {e}")

def brightness_set(value: int):
    """Set brightness to an exact percentage 0-100."""
    value = max(0, min(100, int(value)))
    if _OS == "Darwin":
        # osascript brightness is 0.0-1.0
        frac = value / 100
        subprocess.run(["osascript", "-e",
            f'tell application "System Events" to set brightness of (first display) to {frac}'],
            capture_output=True)
    elif _OS == "Linux":
        if subprocess.run(["which", "brightnessctl"],
                capture_output=True).returncode == 0:
        ...
        try:
            _win_brightness_set(value)
        except Exception as e:
            print(f"[Settings] brightness_set Windows failed: {e}")
...
# ── Action map ────────────────────────────────────────────────────────────────
# Every alias that Gemini might plausibly send is listed here.

ACTION_MAP: dict[str, callable] = {
    # Volume — all spelling variants Gemini might produce
    "volume_up":            volume_up,
    "volumeup":             volume_up,
    ...
    # Brightness — all spelling variants Gemini might produce
    ...
    # Generic "brightness" with no direction → treated as increase (safe default)
...
# ── Intent detection (fallback via Gemini) ────────────────────────────────────
...
- If the user says "increase/raise/turn up volume" use volume_up.
- If the user says "decrease/lower/turn down volume" use volume_down.
- If the user says "increase/raise brightness" use brightness_up.
- If the user says "decrease/lower brightness" use brightness_down.
...
def computer_settings(
    ...
    # If action is vague or missing, ask Gemini what the user wants
